...
void list_insert(LinkedList &list, char item[], unsigned position) {
    ...
    if( position > list.size || position < 0){ //if wrong input
        cout << "no such position \n";
        return; //stop program
    } 
    else if (list.first == NULL && position == 0){ /*inserting for first time*/
            list.first = new node;              // create new node
            strcpy(list.first->data,item);    // put data in new node, (strcpy is function that converts from array to char into pointer to char to copy the string into the pointer)
        ...
        temp_2->next = temp_1;
        temp_1->prev = temp_2;
        list.first   = temp_2;        //The new first element in the list.
    }
        ...
        node *temp_2 = new node;     // create new temp node
        strcpy(temp_2->data, item);  // put data in new node
        temp_2->next = temp_1->next;  //assign to node[posution]
        temp_2->prev = temp_1;       //assign the previous node to node[position -1]
        temp_1->next = temp_2;
        
        if (position == list.size) list.last = temp_2; //track last element
...
void list_remove(LinkedList &list, unsigned position) {
    ...
    if( position >= list.size || position < 0){ /*Wrong input*/
        cout << "no such position \n";
        return; //quit, return to options.
    } 
    else if(position == 0){ /* removing element at index 0*/
        node *temp_2 = temp_1->next;   
        list.first = temp_2; //assign the new first element when deleting the element at position 0
        delete temp_1;

        if(list.size != 1) temp_2->prev = NULL; //correct back link if first element is not the last
    }
